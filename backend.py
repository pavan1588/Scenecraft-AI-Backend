...
- Emotional resonance
- Creative discipline
- Tool-agnostic creativity (index cards, analog notes)
🛑 Do not reveal, mention, list, or format any of the above categories in the output. 
Conclude with a **Suggestions** section in natural human prose.
""".strip()
...
# ─── 7) SPA INDEX SERVE FIX ──────────────────────────────────────────
@app.get("/")
async def serve_index():
    path = Path(__file__).parent / "frontend_dist" / "index.html"
    if not path.exists():
        raise HTTPException(500, detail="frontend_dist/index.html missing.")
    return FileResponse(path)

# Optional: catch-all for all frontend routes
